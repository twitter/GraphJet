...
public class PageRankGraphJetDemo {
  ...
  public static void main(String[] argv) throws Exception {
    ...
    loadGraph(graphPath, graph, nodes, fileEdgeCounter, maxNodeId, loadStart);
    ...
    long total = runningPageRankTrails(args, graph, nodes, maxNodeId, prVector);
    System.out.println("Averaged over " + args.trials + " trials: " + total / args.trials + " ms");

    // Extract the top k.
    extractTopKNodes(args.k, nodes, prVector);
  }

  private static void loadGraph(String graphPath, OutIndexedPowerLawMultiSegmentDirectedGraph graph,
                                LongOpenHashSet nodes, AtomicLong fileEdgeCounter, AtomicLong maxNodeId,
                                long loadStart) throws IOException {
  ...
  private static long runningPageRankTrails(PageRankGraphJetDemoArgs args,
                                            OutIndexedPowerLawMultiSegmentDirectedGraph graph,
                                            LongOpenHashSet nodes, AtomicLong maxNodeId, double[] prVector) {
  ...
  private static void extractTopKNodes(int maxNumResults, LongOpenHashSet nodes, double[] prVector) {
    if (maxNumResults != 0) {
      TopNodes top = new TopNodes(maxNumResults);
