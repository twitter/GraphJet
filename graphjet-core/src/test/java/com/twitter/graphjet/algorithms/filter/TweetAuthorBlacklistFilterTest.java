...
import com.twitter.graphjet.algorithms.filters.TweetAuthorBlacklistFilter;
...
public class TweetAuthorBlacklistFilterTest {
  @Test
  public void testOneAuthor() {
    ...
    tweetAuthors.add(1L);
    TweetAuthorBlacklistFilter authorFilter = new TweetAuthorBlacklistFilter(leftIndexedBipartiteGraph, tweetAuthors, new NullStatsReceiver());

    // Tweets authored by tweetAuthors should be filtered. Node 10 is authored by 1.
    assertEquals(true, authorFilter.filterResult(10, socialProofs));
    // Tweets not authored by tweetAuthors should not be filtered. Node 20 is not authored by 1
